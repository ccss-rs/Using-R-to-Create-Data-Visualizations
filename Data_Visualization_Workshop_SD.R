...
aggregate(mpg ~ cyl, data = mtcars, FUN = mean) #FUN stands for function
...
#Other Results that R will output in a table on your terminal you may want to keep ----
...
#knitr # a package to integrate R code and visualization into documents
...
#create the same table removing row names
kable(head(mtcars), row.names = FALSE, caption = "Table withOUT Row Names")

#create the same table customizing the names of the columns
...
#create the same table with a new caption, with values centered, and in html format
kable(head(mtcars), format = "html", align = "c", caption = "Centered and HTML-formatted Table")


#install.packages('kableExtra') #extension of the kable() function to customize and style tables
library(kableExtra)

# Create a styled table with alternating colors in its rows that grow darker when mouse hovers over it
kable(head(mtcars), format = "html", caption = "Styled Table with kableExtra") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"))

#Same as above but full table
...
summary_stats <- sapply(mtcars, mean) #sapply applies a function to each value of a vector
...
#APA Style is not built in in knitr but here are some things we can do: 
#a header that is centered, 
#centering the content of the table, 
#footnote. 
#Bold and italics require adding that to html output <b></b><i></i>

# Create an APA-style table from the mtcars dataset
apa_table <- kable(mtcars[1:5, 1:4], format = "html", booktabs = TRUE, #booktabs keeps the borders
                   caption = "Table 1\nDescriptive Statistics") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), 
                full_width = FALSE, position = "center") %>% #full_width makes table occupy some or all of the available space
  ...
  footnote(general = "Note. This table provides descriptive statistics for the mtcars dataset.", 
           #general means this footnote applies to the whole table and not a specific row or column
           general_title = "Note.",
           footnote_as_chunk = TRUE) #Independent chunk of text in LaTeX, HTML, or RMarkdown (simplifies formatting)
...
#outputs html, latex, rtf, png, word if you use RMarkdown or Quarto, powerpoint via Officer Package, plain text. 

#no APA styling format
...
basic_table <- mtcars %>% #Pipe operator = passes the result of one expression into the next function as its first argument
...
# Select rows based on condition and format the table
grouped_table_1 <- mtcars %>%
  head(10) %>%  # Display the first 10 rows
  gt() %>%
  tab_header(
    title = "Mtcars Dataset",
    subtitle = "Selected Rows: Cars with Less/More than 6 Cylinders"
  ) %>%
  fmt_number(
...
summary_stats_table <- mtcars %>%
  summarise( #create summar statistics
    Mean_MPG = mean(mpg),
    Mean_HP = mean(hp),
    Max_MPG = max(mpg),
    Max_HP = max(hp),
...
footnote_table_apa <- mtcars %>%
  ...
  tab_footnote(
    ...
    locations = cells_column_labels(columns = c(mpg)) # APA-style note for mpg column
  ) %>%
  tab_source_note(
    source_note = "Source: mtcars dataset from R."
  ) %>%
  tab_style( # Ensure left alignment for APA style
    style = list(
  ...
  tab_options(
    table.border.top.color = "black", # APA requires top border
    table.border.bottom.color = "black", # APA requires bottom border
    table.border.top.width = px(2),
    table.border.bottom.width = px(2),
    heading.border.bottom.color = "black", # Border between header and body
    heading.border.bottom.width = px(1),
    column_labels.border.bottom.color = "black",
    column_labels.border.bottom.width = px(1),
    table.border.left.style = "none", # No vertical lines
    table.border.right.style = "none",
    column_labels.border.top.style = "none",
    table.font.size = px(12), # APA-style table font size
    table.width = pct(100) # Ensures the table is stretched full width
  )
...
#allows for html, latex, word, rtf and plain text outputs
...
apa_summary <- mtcars %>%
  ...
  tbl_summary( #function that creates a descriptive table from a dataset
    label = list(
      mpg ~ "Miles per Gallon",
      cyl ~ "Cylinders",
      disp ~ "Displacement",
  ...
  modify_footnote(
    all_stat_cols() ~ "Mean (SD) values are presented for each variable.",  
    # Add footnote to specify which statistics will be found in this column. uses superscript
    abbreviation = TRUE #add footnotes as descriptions of abbreviations
  ) %>%
...
#grammar of graphics package, data visualization for plots and charts. 

#it can output png, pdf, jpg, tiff

#install.packages("ggplot2")
library(ggplot2)

# Scatter plot of MPG vs Horsepower
ggplot(mtcars, aes(x = hp, y = mpg)) + #aes stand for aesthetics
...
#sjplot visualizes statistical models

#it can output html, word, latex, png, pdf, jpeg
